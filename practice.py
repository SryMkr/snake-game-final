...
# ----------------------------------------------------------------------------------------------------------------------
        ...
        if game_over == True:
            ...
            # show tip
            if tip_show == True:
                print_text(CHINESE_FONT, FRAME_WIDTH * 17, 60,
                           "提示: " + words_list[int(alphabet.current_word_number) - 1],
                           color=(255, 0, 0))
            ...
            print_text(CHINESE_FONT, FRAME_WIDTH * 20, FRAME_WIDTH * 4,
            ...
            if int(alphabet.current_word_number) > 1:
                print_text(CHINESE_FONT, FRAME_WIDTH * 8, FRAME_WIDTH * 4,
                           "刚完成: " + words_list[int(alphabet.current_word_number) - 2])
            # 第一个单词的时候没有任何单词
            else:
            ...
            if ''.join(spell_list) == words_list[int(alphabet.current_word_number) - 2]:
                # delete if do not make mistake and do not use tip 如果完全吃了正确的单词而且没有使用提示
                # 如果拼错了就留在蛇尾
